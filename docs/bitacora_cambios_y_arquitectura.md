...
* **Archivos Creados:**
  - [`docs/manual_flexbox_chess.md`](file:///d:/repos/new/flexbox_chess/docs/manual_flexbox_chess.md): Manual técnico y de usuario del tablero original con HTML5 Drag and Drop y CSS Flexbox.
  - [`docs/requerimientos_sistema.md`](file:///d:/repos/new/flexbox_chess/docs/requerimientos_sistema.md): Especificación de Requerimientos del Sistema (SRS) según estándares IEEE 830.
  - [`docs/especificacion_api_rest_websocket.md`](file:///d:/repos/new/flexbox_chess/docs/especificacion_api_rest_websocket.md): Contrato formal de endpoints REST, esquemas JSON y WebSockets.
  - [`docs/guia_integracion_frontend.md`](file:///d:/repos/new/flexbox_chess/docs/guia_integracion_frontend.md): Guía de enlace entre el DOM del cliente y la API de servidor.
  - [`docs/ia_y_multisesion.md`](file:///d:/repos/new/flexbox_chess/docs/ia_y_multisesion.md): Arquitectura conceptual de inteligencia artificial y juego multi-sesión.
  - [`docs/README.md`](file:///d:/repos/new/flexbox_chess/docs/README.md): Índice unificado de documentación.
...
* **Archivos Creados / Modificados:**
  - [`server.js`](file:///d:/repos/new/flexbox_chess/server.js): Servidor Express en Node.js con endpoints `/api/games`, `/api/status/:id`, `/api/games/:id/moves`, `/api/games/:id/reset` y `/api/games/:id/resign`.
  - [`api/chessEngine.js`](file:///d:/repos/new/flexbox_chess/api/chessEngine.js): Motor de reglas en servidor (validación de casillas, trayectorias de piezas, turnos, generación de notación SAN, conteo de puntos y capturas).
  - [`api/gameStore.js`](file:///d:/repos/new/flexbox_chess/api/gameStore.js): Persistencia con caché en memoria y sincronización atómica en archivos JSON en [`data/games/<id>.json`](file:///d:/repos/new/flexbox_chess/data/games/).
...
* **Archivos Modificados:**
  - [`app/index.html`](file:///d:/repos/new/flexbox_chess/app/index.html): Añadida barra superior con selector de Partida ID, insignias de estado de la API, selector de bando (`⚪ Blancas`, `⚫ Negras`, `⚪⚫ Ambos`) y modo de juego.
  - [`app/js/index.js`](file:///d:/repos/new/flexbox_chess/app/js/index.js): Implementado bucle `startSyncLoop()` que sondea el estado cada 1200ms comparando el turno y contador de jugadas, evitando renders innecesarios.
  - [`app/js/chess-rules.js`](file:///d:/repos/new/flexbox_chess/app/js/chess-rules.js): Modificada la función `drop(ev)` para emitir `POST /api/games/:id/moves` antes de consolidar el movimiento en la vista del cliente.
...
* **Archivos Creados / Modificados:**
  - [`api/chessAI.js`](file:///d:/repos/new/flexbox_chess/api/chessAI.js): Algoritmo Minimax con Poda Alfa-Beta y evaluación posicional mediante *Piece-Square Tables*.
  - [`server.js`](file:///d:/repos/new/flexbox_chess/server.js): Endpoint `POST /api/games/:id/bot-move` para solicitar cálculo del robot.
  - [`app/js/index.js`](file:///d:/repos/new/flexbox_chess/app/js/index.js): Función `triggerBotMove()` que detecta automáticamente cuando es el turno del robot y ejecuta su jugada con un delay natural de 600ms.
...
* **Archivos Modificados:**
  - [`app/js/index.js`](file:///d:/repos/new/flexbox_chess/app/js/index.js): Implementación de `renderMovementsTable()` y `renderMovementsPagination()` con lógica matemática descendente:
    - Página 1 muestra las jugadas más recientes (ej. `#37` a `#28`).
    - Las páginas posteriores muestran las jugadas anteriores (ej. `#27` a `#18`).
  - [`app/index.html`](file:///d:/repos/new/flexbox_chess/app/index.html): Añadido contenedor `#movements-pagination`.
  - [`app/css/index.css`](file:///d:/repos/new/flexbox_chess/app/css/index.css): Estilos CSS modernos para botones activos, deshabilitados y transiciones.
...
* **Archivos Modificados:**
  - [`server.js`](file:///d:/repos/new/flexbox_chess/server.js): Endpoint `GET /api/games/:id/history/:step` que reconstruye el tablero en cualquier turno `step`.
  - [`api/chessEngine.js`](file:///d:/repos/new/flexbox_chess/api/chessEngine.js): Función `getBoardAtStep(movements, step)`.
  - [`app/index.html`](file:///d:/repos/new/flexbox_chess/app/index.html): Controles de navegación `|◀` (Inicio), `◀ Anterior`, indicador central, `Siguiente ▶`, `▶|` (En vivo) y banner naranja de advertencia.
  - [`app/js/index.js`](file:///d:/repos/new/flexbox_chess/app/js/index.js): Funciones `navigateHistory()`, `enterHistoryMode()`, `exitHistoryMode()`, filas de la tabla de movimientos clicables para saltar directamente a una jugada y piezas con `draggable="false"`.
  - [`app/js/chess-rules.js`](file:///d:/repos/new/flexbox_chess/app/js/chess-rules.js): Bloqueo estricto en `drop(ev)` si `isHistoryMode === true`.
...
* **Archivos Modificados:**
  - [`api/chessEngine.js`](file:///d:/repos/new/flexbox_chess/api/chessEngine.js):
  ...
  - [`server.js`](file:///d:/repos/new/flexbox_chess/server.js): Expone `winner: game.winner || null` e `in_check: game.in_check || false`.
  - [`app/js/index.js`](file:///d:/repos/new/flexbox_chess/app/js/index.js) y [`app/js/chess-rules.js`](file:///d:/repos/new/flexbox_chess/app/js/chess-rules.js):
...
* **Archivos Modificados:**
  - [`api/chessAI.js`](file:///d:/repos/new/flexbox_chess/api/chessAI.js):
  ...
  - [`server.js`](file:///d:/repos/new/flexbox_chess/server.js):
  ...
  - [`app/index.html`](file:///d:/repos/new/flexbox_chess/app/index.html):
    - Desplegable `#bot-difficulty` con los 10 niveles identificados con su nombre y estilo.
  - [`app/js/index.js`](file:///d:/repos/new/flexbox_chess/app/js/index.js):
...
* **Archivos Creados / Modificados:**
  - [`api/userStore.js`](file:///d:/repos/new/flexbox_chess/api/userStore.js) [NUEVO]:
    - Almacén de usuarios con persistencia en disco en [`data/users/<id>.json`](file:///d:/repos/new/flexbox_chess/data/users/).
  ...
  - [`api/gameStore.js`](file:///d:/repos/new/flexbox_chess/api/gameStore.js):
  ...
  - [`app/css/index.css`](file:///d:/repos/new/flexbox_chess/app/css/index.css):
...
|---|---|---|
| **`server.js`** | [`server.js`](file:///d:/repos/new/flexbox_chess/server.js) | Servidor API Express, middleware de autenticación por Bearer token, rutas `/api/auth/*` y `/api/users/*`. |
| **`userStore.js`** | [`api/userStore.js`](file:///d:/repos/new/flexbox_chess/api/userStore.js) | Almacén de usuarios, derivación PBKDF2 + salt criptográfico, sesiones, estadísticas Elo e historial filtrado. |
| **`chessEngine.js`** | [`api/chessEngine.js`](file:///d:/repos/new/flexbox_chess/api/chessEngine.js) | Reglas oficiales del ajedrez, cálculo de jaque, jaque mate, tablas, reconstrucción histórica y asociación de jugadores. |
| **`chessAI.js`** | [`api/chessAI.js`](file:///d:/repos/new/flexbox_chess/api/chessAI.js) | Algoritmo Minimax, Poda Alfa-Beta, Quiescence, Move Ordering y los 10 niveles de dificultad. |
| **`gameStore.js`** | [`api/gameStore.js`](file:///d:/repos/new/flexbox_chess/api/gameStore.js) | Persistencia dual (Memoria + Archivos JSON en `data/games/`) y actualización automática de estadísticas al terminar. |
| **`data/users/`** | [`data/users/`](file:///d:/repos/new/flexbox_chess/data/users/) | Directorio de persistencia JSON de perfiles de usuario, credenciales hasheadas y estadísticas acumuladas. |
| **`index.html`** | [`app/index.html`](file:///d:/repos/new/flexbox_chess/app/index.html) | Estructura DOM, barra de sesión en header, modal de login/registro, modal de partidas por usuario y paginador. |
| **`index.js`** | [`app/js/index.js`](file:///d:/repos/new/flexbox_chess/app/js/index.js) | Gestión de sesión/tokens en localStorage, interfaz de usuario autenticado, carga de partidas de usuario y paginación. |
| **`chess-rules.js`** | [`app/js/chess-rules.js`](file:///d:/repos/new/flexbox_chess/app/js/chess-rules.js) | Controladores de eventos de arrastre con token de autenticación en cabeceras HTTP y validación de turnos. |
| **`index.css`** | [`app/css/index.css`](file:///d:/repos/new/flexbox_chess/app/css/index.css) | Estilos visuales del tablero, modales accesibles, tarjetas de estadísticas, tablas de partidas e insignias. |
